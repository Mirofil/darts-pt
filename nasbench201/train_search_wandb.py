...
parser.add_argument('--merge_train_val', type=lambda x: False if x in ["False", "false", "", "None"] else True, default=False, help='fine tune epochs between two projections')
...
def main():
    ...
    if not torch.cuda.is_available():
        ...
        sys.exit(1)
    run = wandb.init(project="NAS", group=f"Search_Cell_PTDarts", reinit=True)
    ...
    if not args.merge_train_val:
        valid_queue = torch.utils.data.DataLoader(
            ...
            pin_memory=True)
    else: # The difference is in the split used
        valid_queue = torch.utils.data.DataLoader(
            train_data, batch_size=args.batch_size,
            sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[:split]),
            pin_memory=True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if not args.fast:
            ...
            wandb.log({**acc_log, **loss_log, **true_acc_log, "epoch": epoch})
